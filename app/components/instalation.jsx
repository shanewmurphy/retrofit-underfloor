export default function Instalation() {
  return (
    <div className="w-11/12 pt-24 py-24 mx-auto">
      <div className="grid lg:grid-cols-2 lg:gap-16 antialiased">
        <div>
          <h4 className="text-zinc-800 lg:text-4xl md:text-5xl sm:text-3xl sm:mb-4 font-bold">
            Installing Underfloor Heating with our Concrete Milling Machine
            Method
          </h4>
        </div>
        <div>
          <p className="text-lightgrey text-base font-medium">
            With this method, we create tightly routed grooves in existing floor
            constructions, into which the underfloor heating pipes are laid.
            This allows for a certified water-based retro-fit underfloor heating
            system that is quick, affordable, and dust-free.
          </p>
        </div>
      </div>
      <div className="grid lg:grid-cols-3 gap-12 mt-16">
        <div>
          <div>
            <img
              loading="lazy"
              srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/3fcf72d78ceaf519d387e47a508d5e9bb829a138e120aaab4751325d5b30019c?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
              className="aspect-[1.38] object-contain object-center w-full overflow-hidden rounded-lg"
              alt=""
            />
            <div>
              <h6 className="text-zinc-800 text-xl font-bold leading-7 mt-4 mb-2">
                Our Custom Floor Cutting for Your Needs
              </h6>
              <p className="text-sm sm:text-base font-medium text-lightgrey">
                We offer bespoke floor cutting on-site to ensure a perfect fit
                for your underfloor heating system.
              </p>
            </div>
          </div>
        </div>
        <div>
          <div>
            <img
              loading="lazy"
              srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/83c7d91ff6581aa0c1b49a98f44a1950b038a5b4204250a9c5618357a3268e45?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
              className="aspect-[1.39] object-contain object-center w-full overflow-hidden"
            />
          </div>
          <div>
            <h6 className="text-zinc-800 text-xl font-bold leading-7 mt-4 mb-2">
              Seamless Integration with Existing Systems
            </h6>
            <p className="text-sm sm:text-base  font-medium text-lightgrey">
              Our underfloor heating system seamlessly integrates with existing
              heating systems, making it easy to combine and reducing energy
              usage and bills.
            </p>
          </div>
        </div>
        <div>
          <div>
            <img
              loading="lazy"
              srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/5322dabf9e5d64c8ba2e01911ef8bbcb295a01a3415eb07d195c072c22b5bad1?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
              className="aspect-[1.39] object-contain object-center w-full overflow-hidden rounded-lg"
            />
          </div>
          <div>
            <h6 className="text-zinc-800 text-xl font-bold leading-7 mt-4 mb-2">
              Feel the Warmth In Every Corner of the Room
            </h6>
            <p className="text-sm sm:text-base font-medium text-lightgrey">
              Experience the comfort of our underfloor heating system as it
              warms up your space in minutes.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
